...
namespace tinyjson {
    ...
    char Parser::GetNextToken() {
        while (std::isspace(str_[idx_]) != 0) { ++idx_;
}
    ...
    JObject Parser::ParseNumber() {
        ...
        // 判断有没有小数部分，没有就直接返回
        if (str_[idx_] != '.') {
            return (static_cast<int>(strtol(str_.c_str() + begin, nullptr, 10)));
        }
        // 处理小数部分
        if (str_[idx_] == '.') {
            idx_++;
        ...
        return (strtof64(str_.c_str() + begin, nullptr));
    }
    ...
    void Parser::TrimRight() {
        // 去除尾部空字符，方便最后的结束检查
        str_.erase(std::find_if(str_.rbegin(), str_.rend(), [](char ch) {
